define(['jquery', 'angular', 'jsTree', 'ngJsTree', 'service/enumService', 'service/enumFilter','views/forbid/filter/filter','views/forbid/directive/passTimeRange','views/forbid/directive/timeRangeSlider','views/forbid/directive/viewForbidRange'], function () {
    // controller
            ...
            $scope.dtOptions1 = {
                ...
                "ajax": function (source, callback, settings) {
                    ...
                    if(params.status=="" || !params.status){
                    	params.status = ["IN_REVIEW","TO_PRINT","PRINTED","RENTERNED"];
                    }
                ...
                "rowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    $(nRow).bind('click', function () {
                        ...
                        UtilService.httpRequest("jwt_forbid/forbid/card/long_card/reviewDetail", "GET", {
	                        ...
	                        $scope.$apply(function () {
	                            $scope.detailPassAreaList = rData.passAreaList;
	                            $timeout(function(){
	                            	$scope.instance.init();
	                            	$scope.instance.redraw(rData.passAreaList);
	                            });
	                        });
	                        
                        });
                        
                        UtilService.httpRequest('jwt_forbid/forbid/card/long_card/reviewList','GET',{passCardId: aData.passCardId},function(data){
...
//					plate:$scope.cardHistoryDetail.plate
            ...
            $scope.addCarHistroy = function(){
            	...
            	if($scope.passPeriodData.sourceId != "-2" && $scope.passPeriodData.sourceId != "-1"){
            		params.passTimeRangeValue = $scope.passPeriodData.passTimeRangeValue; //常用时段段
            	}
