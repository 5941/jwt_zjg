...
'views/forbid/directive/forbidAreaMap',
'views/forbid/directive/passTimeRange',
'views/forbid/directive/timeRangeSlider'], function () {
    // controller
        ...
        function ($rootScope, $scope, $state, $log, UtilService, $timeout,$compile,notify,$q) {
            ...
            $scope.next = function(){
                ...
                if($scope.tab_view == "info"){
                    ...
                    //批量提交通行范围
                    UtilService.httpRequest('jwt_forbid/forbid/card/pass_area/add', 'POST',passAreas, function(data){
                        for(var i in $scope.params1){
                            $scope.params1[i].areaIdList = angular.copy(data.data);
                            $scope.params1[i].deptIdList = $scope.deptIdList;
                        }
                        console.log($scope.params1);
                        //批量提交通行证
                        UtilService.httpRequest('jwt_forbid/forbid/card/long_card/add', 'POST',$scope.params1, function(data){
                            UtilService.toastSuccess("提交成功");
                        });
                    });
                }
            }
            ...
            $scope.loadPassInfo = function(){
                ...
                var promise = function(){
                    ...
                    UtilService.httpRequest('jwt_forbid/forbid/card/long_card/page', 'POST',params, function(data){
                        ...
                        // if(passCard.passTimeRangeID)
                        //     $scope.passPeriodData = passCard.passTimeRangeID;
                        // if(passCard.passTimeRangeID == -2)
                        //     $scope.rangeList = passCard.passTimeRangeValue;
            ...
            $scope.addDefPassArea = function(){
                console.log($scope.data.passAreas);
            ...
            $scope.selectAllDutyArea = function(){
                for(var i in $scope.duty_areas){
                    $scope.duty_areas[i].selected = $scope.allDutyAreaSelect;
                }
            }
